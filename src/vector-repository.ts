...
export interface HierarchicalNSWIndex {
  ...
  searchKnn(
    query: Float32Array,
    k: number,
    filter?: ((label: number) => boolean) | number[]
  ): { distances: number[]; neighbors: number[] }; // Expect Float32Array
...
export class VectorRepository implements VectorRepositoryInterface {
  ...
  private async init(): Promise<void> {
    ...
    try {
      ...
      try {
        ...
        this.vectorIndex = new hnswlib.HierarchicalNSW(
          'cosine',
          this.dimensions
        ) as unknown as HierarchicalNSWIndex;
        ...
        if ((await fs.pathExists(indexPath)) && (await fs.pathExists(mapPath))) {
          try {
            ...
            console.error(
              `Loaded vector index with ${this.vectorIndex.getCurrentCount()} elements (max: ${this.vectorIndex.getMaxElements()}). Next label: ${this.nextIndex}`
            );
          } catch (loadError) {
  ...
  public async ensureInitialized(): Promise<void> {
    ...
    if (this.fallbackMode) {
      ...
      if (process.env.NODE_ENV !== 'test') {
        console.warn(
          'Vector repository is in fallback mode, some operations may not work as expected'
        );
      }
    }
  }
  ...
  private async saveFallbackStorage(): Promise<boolean> {
    ...
    try {
      ...
      const fallbackObject = Array.from(this.fallbackStorage.entries()).reduce(
        (obj, [key, value]) => {
          obj[key] = value;
          return obj;
        },
  ...
  public async addSummary(summary: ContextSummary): Promise<void> {
    ...
    try {
      ...
      if (!textToEmbed) {
        console.error(
          `Summary text is missing for context ${summary.contextId}, cannot generate embedding.`
        );
        return;
      }
      ...
      if (!embedding) {
        console.error(
          `Failed to generate embedding for context ${summary.contextId}, cannot add to index.`
        );
        return;
      }
      ...
      if (labelToAdd === undefined) {
        ...
        console.error(
          `Added context ${summary.contextId} to vector index with label ${labelToAdd}`
        );
      } else {
        console.error(
          `Context ${summary.contextId} exists (label ${labelToAdd}). Updating by replacement.`
        );
        try {
          ...
          console.error(
            `Error updating index for ${summary.contextId} (label ${labelToAdd}): ${updateError.message}.`
          );
        }
      }
      ...
      console.error(
        `Error processing addSummary for context ${summary.contextId}: ${error.message}`,
        error.stack
      );
    }
  }
  ...
  public async findSimilarContexts(text: string, limit: number = 5): Promise<SimilarContext[]> {
    ...
    if (!this.vectorIndex || !this.embeddingModel || this.contextIdToIndex.size === 0) {
      console.error(
        'Vector index/model not ready or index empty, cannot perform similarity search.'
      ); // Log to stderr
      return [];
    }
    ...
    try {
      ...
      for (let i = 0; i < neighbors.length; i++) {
        ...
        if (contextId) {
          ...
          console.warn(
            `Warning: Found neighbor label ${neighborLabel} with no corresponding contextId.`
          ); // Log to stderr
        }
      }
  ...
  public async deleteContext(contextId: string): Promise<void> {
    ...
    try {
      ...
      if (this.fallbackMode) {
        ...
        if (!saveResult) {
          console.warn(
            `Failed to update fallback storage after deleting '${contextId}'. Data might be inconsistent on restart.`
          );
        }
