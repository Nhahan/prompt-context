...
import { Message, ContextData, ContextSummary, SimilarContext, ContextRelationshipType, ContextImportance, MCPConfig, SummaryResult, ApiCallType } from '../types';
...
export class ContextService {
    ...
    async addMessage(contextId: string, messageData: Omit<Message, 'timestamp'>): Promise<void> {
        console.log(`[ContextService] Adding message to ${contextId}`);
        ...
        try {
            ...
            if (this.config.autoSummarize && this.summarizer && newMsgCount >= (this.config.messageLimitThreshold || 10)) {
                ...
                this.triggerBackgroundSummarization(contextId).catch(err => {
                    ...
                    console.error(`[ContextService] Error initiating background summarization for ${contextId}:`, err);
                });
            }
        } catch (error: unknown) {
    ...
    async getContext(contextId: string): Promise<ContextData | null> {
        console.log(`[ContextService] Getting context for ${contextId}`);
        try {
    ...
    async triggerManualSummarization(contextId: string): Promise<SummaryResult> {
        const stopTracking = this.analytics?.trackCall(ApiCallType.LLM_SUMMARIZE, { contextId, manualTrigger: true });
        ...
        try {
            ...
            if (!messages || messages.length === 0) {
                console.warn(`[ContextService] No messages found for ${contextId}, skipping summarization.`);
            ...
            console.log(`[ContextService] Manual Summarization: Summarizer returned ${result.success ? 'success' : 'failure'}`);
            ...
            if (result.success && result.summary) {
                ...
                if (this.repositories.vector) {
                    console.log(`[ContextService] Manual Summarization: Adding summary to vector DB for ${contextId}`);
                    try {
                        ...
                        console.error(`[ContextService] Error adding summary to vector DB for ${contextId}:`, vectorError);
                ...
                if (!existingMetadata) {
                    ...
                    console.error(`[ContextService] Metadata not found for ${contextId} after loading messages. Cannot update.`);
                ...
                console.error(`[ContextService] Manual summarization failed for ${contextId}: ${result.error}`);
            }
            return result;
        } catch (error) {
            const errorMsg = error instanceof Error ? error.message : String(error);
            console.error(`[ContextService] Error during manual summarization for ${contextId}:`, errorMsg);
    ...
    async findSimilarContexts(query: string, limit: number): Promise<SimilarContext[]> {
        const stopTracking = this.analytics?.trackCall(ApiCallType.VECTOR_DB_SEARCH, { queryLength: query.length, limit });
        console.log(`[ContextService] Finding similar contexts for query: ${query.substring(0, 30)}...`);
        if (!this.repositories.vector) {
    ...
    async addRelationship(sourceContextId: string, targetContextId: string, relationshipType: ContextRelationshipType, weight: number): Promise<void> {
        const stopTracking = this.analytics?.trackCall(ApiCallType.GRAPH_DB_ADD, { type: relationshipType, weight });
        console.log(`[ContextService] Adding relationship ${sourceContextId} -> ${targetContextId}`);
        if (!this.repositories.graph) {
            throw new Error('Graph repository is not available.');
        }
        // 1. Call graph repository
        await this.repositories.graph.addRelationship(sourceContextId, targetContextId, relationshipType, weight);
    }

    async getRelatedContexts(contextId: string, relationshipType?: ContextRelationshipType, direction?: 'incoming' | 'outgoing' | 'both'): Promise<string[]> {
        const stopTracking = this.analytics?.trackCall(ApiCallType.GRAPH_DB_SEARCH, { type: relationshipType, direction });
        console.log(`[ContextService] Getting related contexts for ${contextId}`);
        if (!this.repositories.graph) {
            throw new Error('Graph repository is not available.');
        }
        // 1. Call graph repository
        const results = await this.repositories.graph.getRelatedContexts(contextId, relationshipType, direction);
    ...
    private async triggerBackgroundSummarization(contextId: string): Promise<void> {
        const stopTracking = this.analytics?.trackCall(ApiCallType.LLM_SUMMARIZE, { contextId, manualTrigger: false });
        ...
        try {
            ...
            if (shouldSummarize && metadata) { // Ensure metadata exists before updating
                ...
                if (result.success && result.summary) {
                    ...
                    console.error(`[ContextService] Background Summarization: Failed for ${contextId}. Error: ${result?.error}`);
                }
            }
        } catch (bgError: unknown) {
            const errorMessage = bgError instanceof Error ? bgError.message : String(bgError);
            console.error(`[ContextService] Background summarization process failed for ${contextId}:`, errorMessage);
        } finally {
            if (stopTracking) stopTracking();
        }
    }
